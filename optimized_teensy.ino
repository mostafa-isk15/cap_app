...
bool canExecuteCommand(String command) {
  // Handle numeric commands (single digits 1-9)
  if (command.length() == 1) {
    char c = command.charAt(0);
    if (c >= '1' && c <= '9') {
      return true;
  ...
  // Validate command against known list and PRESET prefix
  bool recognized = command.startsWith("PRESET");
  if (!recognized) {
